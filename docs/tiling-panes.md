...
| **Drag** | Each pane in a split carries a name bar along its bottom edge — its `Alt+<n>` number, its name, and the pop-out button — and the whole bar is the drag handle. It replaced a corner grip that appeared on hover: dragging inside the terminal already means "select text", so the drag source has to be something else, and a strip the full width of the pane is a target you cannot miss. It also gives the panes names, which a split otherwise has nowhere to put — the tab strip can only ever name the focused one. Drop on another pane's edge to land in that half; drop in its middle to swap the two; drop on the tab strip to pull the pane out into its own tab. |
